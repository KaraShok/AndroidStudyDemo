...
public class PluginManager {
    ...
    public void loadPlugin(Context context, String path) {
        ...
        // activitys
        File dex = context.getDir("dex", Context.MODE_PRIVATE); // 缓存目录
  ...
  private void parseReceivers(Context context, String path) {
      try {
          ...
          Object packageObj =
                  parsePackageMethod.invoke(packageParser, new File(path), PackageManager.GET_ACTIVITIES);
          ...
          Method generateReceiverInfo =
                  packageParserClass.getDeclaredMethod(
          ...
          for (Object activity : receivers) {
              ActivityInfo info =
                      (ActivityInfo)
                              generateReceiverInfo.invoke(packageParser, activity, 0, defaltUserState, userId);
